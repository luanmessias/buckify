...
describe("MonthSelector Component", () => {
	...
	it("should open drawer/popover and show year and months", async () => {
		...
		expect(
			within(dialog).getByRole("link", { name: /jan/i }),
		).toBeInTheDocument()
		expect(
			within(dialog).getByRole("link", { name: /mai/i }),
		).toBeInTheDocument()
	})
	...
	it("should select a month and update URL", async () => {
		...
		const janButton = within(dialog).getByRole("link", { name: /jan/i })
	...
	it("should disable future months", async () => {
		...
		const julyButton = within(dialog).getByRole("link", { name: /jul/i })
		expect(julyButton).toHaveAttribute("disabled")
	})
})
